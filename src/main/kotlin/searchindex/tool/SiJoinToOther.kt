...
open class SiJoinToOther<T1: SiRecord, T2: SiRecord>(
    ...
    override suspend fun all(start: Int, rows: Int): SiHits<SiOneToOther<T1, T2>> {
    ...
    private suspend fun toOneToOther(one: T1): SiOneToOther<T1, T2>? = (one.getValue(foreignKeyField) as String?)?.let { foreignKey ->
        val other = secondary.byId(foreignKey)
        SiOneToOther<T1, T2>(one, other)
    }

    suspend fun toOneToOtherList(oneList: List<T1>): List<SiOneToOther<T1, T2>> =
        oneList.mapNotNull { one -> toOneToOther(one) }


    override suspend fun search(query: Query, start: Int, rows: Int, sort: Sort?): SiHits<SiOneToOther<T1, T2>> {
    ...
    override suspend fun byId(q: String): SiOneToOther<T1, T2>? = primary.byId(q)?.let {
        toOneToOther(it)
    }
